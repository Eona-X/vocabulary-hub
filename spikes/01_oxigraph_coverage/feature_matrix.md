...
|---|---|---|---|---|---|---|
| C01 | SPARQL 1.1 SELECT | ADR-001 | `probe: sparql-select` | _tbd_ | _tbd_ | basic conjunctive |
| C02 | SPARQL 1.1 CONSTRUCT | ADR-001 | `probe: sparql-construct` | _tbd_ | _tbd_ | |
| C03 | SPARQL 1.1 ASK | ADR-001 | `probe: sparql-ask` | _tbd_ | _tbd_ | |
| C04 | SPARQL 1.1 DESCRIBE | ADR-001 | `probe: sparql-describe` | _tbd_ | _tbd_ | |
| C05 | SPARQL 1.1 UPDATE — INSERT DATA | ADR-001 | `probe: sparql-update-insert` | _tbd_ | _tbd_ | |
| C06 | SPARQL 1.1 UPDATE — DELETE WHERE | ADR-001 | `probe: sparql-update-delete` | _tbd_ | _tbd_ | |
| C07 | SPARQL 1.1 Graph Store Protocol | ADR-001 | `probe: gsp-put-get` | _tbd_ | _tbd_ | |
| C08 | SPARQL 1.1 Federation (SERVICE) | ADR-001 | `probe: sparql-service` | _tbd_ | _tbd_ | ADR-004 flagged as gap-to-verify |
| C09 | Property paths | SPARQL 1.1 | `probe: sparql-paths` | _tbd_ | _tbd_ | |
| C10 | Named graphs / quads | RDF 1.1 | `probe: named-graphs` | _tbd_ | _tbd_ | |
| C11 | Turtle parse + serialize | README | `probe: io-turtle` | _tbd_ | _tbd_ | |
| C12 | JSON-LD parse + serialize | README | `probe: io-jsonld` | _tbd_ | _tbd_ | |
| C13 | N-Triples parse + serialize | README | `probe: io-ntriples` | _tbd_ | _tbd_ | |
| C14 | RDF/XML parse + serialize | README | `probe: io-rdfxml` | _tbd_ | _tbd_ | |
| C15 | TriG / N-Quads | RDF 1.1 | `probe: io-trig` | _tbd_ | _tbd_ | bulk-load formats listed in ADR-004 §1 |
| C16 | Content negotiation on SPARQL endpoint | README | `probe: conneg` | _tbd_ | _tbd_ | |
| C17 | SKOS concept-scheme query (skos:broader transitive) | IDS-RAM §3.5.6 | `probe: skos-broader` | _tbd_ | _tbd_ | |
| C18 | Full-text index | ADR-004 §1 gap | `probe: text-search` | _tbd_ | _tbd_ | ADR-004 calls out as gap-to-verify |
...
| C23 | Persistent on-disk store | ADR-001 | `probe: persistence-restart` | _tbd_ | _tbd_ | Oxigraph: RocksDB |
| C24 | Bulk load (≥ 1M triples) | ADR-004 §1 | `probe: bulk-load` | _tbd_ | _tbd_ | timed separately in Spike 2 |
